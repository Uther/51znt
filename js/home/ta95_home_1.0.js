...
usingNamespace("Web.UI")["Layer"] = {
    ...
    Open: function() {
        ...
        if ($("#" + $Layer.ID).length == 0) {
            ...
            var confirmContent = ""; if ($Layer.IsConfirm) { confirmContent = "<input type='button' onclick='" + $Layer.Confirm.Event + "' value='" + $Layer.Confirm.Text + "' id='btnLayerEnter' class='buttons'/>"; }
            // window width
    ...
    OpenCustom: function() {
        ...
        if ($("#" + $Layer.ID).length == 0) {
            ...
            var footer = ""; if ($Layer.IsFooter) { footer = "<div style='background:#EEE;text-align:center;padding:5px 10px;'>" + confirmContent + "<input type='button' onclick='$Layer.Close();' value='" + $Layer.CloseText + "' id='btnLayerClose' class='buttons'/></div>"; }
            var element = document.documentElement;
            $("#" + $Layer.ID).append(content + footer);
            // 设置window居中显示
            top = (element.clientHeight - $("#" + $Layer.ID).height()) / 2;
            if (top < 0) {
                top = 0;
...
usingNamespace("Web")["Common"] = {
	...
	AlertAuto: function(msg, width, title, btnTitle) {
        $Layer.IsBack = false;
        $Layer.Width = width != null ? width : 500;
        $Layer.Title = title || "提示";
        $Layer.IsFooter = false;
        $Layer.IsConfirm = false;
		$Layer.Confirm = {};
		$Layer.IsAutoClose = true;
		$Layer.AutoTime = 1000;
        var btnTitle = btnTitle || "关闭";
        var msgContent = "<div class='cont'>";
        msgContent += '<iframe scrolling="no" frameborder="0" style="z-index:-1;position:absolute; left:8px; top:10px; width:' + (width-20) + 'px;height:80px;allowtransparency="true;"></iframe>';
        msgContent += "<div class='tipInfoA'>" + msg + "</div>";
        //msgContent += "<div class='toDoBox'><button class='buttons' type='button' onclick='$Layer.Close();return false;'>" + btnTitle + "</button></div>";
        msgContent += "<div class='c'></div>";
        $Layer.Content = msgContent;
	...
	Confirm: function(msg, width, func){
		$Layer.IsBack = true;
        $Layer.Width = width != null ? width : 500;
        $Layer.Title = "系统确认";
        $Layer.IsFooter = true;
    ...
    OpenCustomLayer: function(url, width, heith) {
        $Layer.Reset();
        $Layer.IsBack = true;
        $Layer.IsFooter = false;
        $Layer.Width = width != "undefined" ? width : 500;
...
function openAppendDomain()
 ...
 {
     $Common.OpenLayer('供求询盘', 'index.php?r=home/messageBox/Send/Inquiry&tid=' + tid, 500, 220);
     return false;
 } 
 //zz询盘
 ...
 //写入用户中心菜单
 function setNowMySpaceMenu(key,value)
  {
      var date = new Date();    
...
function homeTabComm(tabs, contents, obj)
{
	var cur = 'cur';	//默认当前样式
...
var MenuAction = 
{
	show: function(obj_id, obj)
	{
		var tclass = $(obj).attr('class');
		if(tclass == 'tree_minus')
		{
			$(obj).removeClass('tree_minus');
			$(obj).addClass('tree_add');
		} else {
			$(obj).removeClass('tree_add');
			$(obj).addClass('tree_minus');
		}
		$(obj_id).toggle();
	}
}
...
function signIn(obj)
{
	$.post('index.php?r=home/circlesHome/weiBo/signIn',null,function(data){
		eval("var ans=" + data);
		if(ans.status == 1)
		{
			$Common.AlertAuto('签到成功，积分 +' + ans.point, 300, '提示' );
			$("#mypoint").html(ans.count);
			$(obj).removeClass('signin').addClass('signout');
		} else if(ans.status == -1) {
			$Common.AlertAuto('今天已经领取过了', 300, '温馨提示' );
		} else {
			$Common.AlertAuto('签到失败', 300, '温馨提示' );
		}
	});
}
function feedback()
{
	...
	if(text == '')
	{
		$Common.AlertAuto('内容不能为空', 300, '温馨提示' );
		return false;
	}
	
	$.post('index.php?r=home/circlesHome/weiBo/feedback',{text:text},function(data){
		if(data == 1)
		{
			$Common.AlertAuto('您的意见已经提交，我们会尽快处理', 300, '提示' );
			$("#feedback").val('');
		} else if(data == 0) {
			$Common.AlertAuto('请先登录', 300, '温馨提示' );
		}
	});
}
function expandContent(obj,id)
{
	var lang = $(obj).attr('lang');
	if(lang=='off'){
		$('#'+id).css({overflow:'visible',height:'auto'});
		$(obj).html("收起");
		$(obj).attr('lang','on');
	}else{
		$('#'+id).css({overflow:'hidden',height:'101px'});
		$(obj).html("展开");
		$(obj).attr('lang','off');
	}
}

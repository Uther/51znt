   ...
   $.each(nodes,function(n,node){  
	   ...
	   if(node.name.indexOf('sort') > -1)
		 {                                 
			...
			pars+= tnode[0] + "_" + node.value + "__"; 
		 }  
   });
   
   
					
   $.post(urlAddr,{id:pars},function(msg){ 
								alert(msg); 
...
function editForm(id,title,url,width,height)
{
	width = width || 800;
	height = height || 560;
	window.top.art.dialog(
		{id:id,iframe:url, title:title, width:width, height:height}, 
		function(){

...
dhtmlx.chart.bar = {
	pvt_render_bar: function(a, b, c, d, e, f) {
		...
		g = i.max;
		i = i.min;
		var n = Math.floor((d.x - c.x) / b.length); ! e && !(this.e.origin != "auto" && !m) && this.N(a, b, c, d, i, g, n);
		if (m) {
			g = parseFloat(this.e.yAxis.end);
			i = parseFloat(this.e.yAxis.start)
		}
...
dhtmlx.Type = {
	add: function(a, b) {
		...
		this.T(b, "edit");
		this.T(b, "loading");
		a.types[c] = dhtmlx.extend(dhtmlx.extend({},
		a.types[c] || this.ta), b);
		return c
	},
	ta: {
		...
		dhtmlx.compat.dataProcessor = function(c) {
			...
			c[d] = function(i, j) {
				if (i) {
					...
					this[f] != "POST" ? dhtmlx.ajax().get(h + (h.indexOf("?") != -1 ? "&": "?") + this.serialize(i, j), "",
					function(m, l) {
						b.xml = dhtmlx.DataDriver.xml.checkResponse(m, l);
						k.afterUpdate(k, null, null, null, b)
					}) : dhtmlx.ajax().post(h, this.serialize(i, j),
					function(m, l) {
						b.xml = dhtmlx.DataDriver.xml.checkResponse(m, l);
						k.afterUpdate(k, null, null, null, b)
					});
...
dhtmlx.DataLoader = {
	...
	load: function(a, b, c) {
		...
		if (!this.data.feed) this.data.feed = function(d, e) {
			...
			this.load(a + (a.indexOf("?") == -1 ? "?": "&") + "posStart=" + d + "&count=" + e,
			function() {
				var f = this.F;
				this.F = false;
...
dhtmlx.DataDriver.dhtmlxgrid = {
	...
	getDetails: function(a) {
		for (var b = {},
		c = 0; c < this.ea.getColumnsNum(); c++) b["data" + c] = this.ea[this.Ia](a, c);
		return b
	},
	getInfo: function() {
		return {
...
dhtmlx.Canvas = {
	...
	renderText: function(a, b, c, d, e, yp) {
		if (c) {
	...
	renderTextAt: function(a, b, c, d, e, f, g) {
		if (e = this.renderText.call(this, c, d, e, f, g, a)) {
...
dhtmlXChart.prototype = {
		...
		if (this.e.yAxis) {
			...
			if (g !== 0) {
				...
				for (var m = 0, l = e; l <= f; l += g) {
					...
					var n = Math.floor(j - m * k) + 0.5; ! (l == e && this.e.origin == "auto") && this.e.yAxis.lines && this.i(a, i, n, d.x, n, this.e.yAxis.color, 0.2);
	...
	Ba: function(a, b, c, d, e) {
		this.e.xAxis && this.renderTextAt(e, d, a, b, this.e.xAxis.template(c))
	},
	Ca: function(a, b, c, d) {
	...
	za: function(a, b) {
		...
		f,
		g,
		i = this.e.legend.layout != "x" ? "width:" + e.width + "px": "",
